 ...
 * 
 * 7/7/05 write menuloader.js from lib jar file instead or RequestDispatch
 * 11/9/05 add logging-in logic 5/17/06 add printPage 12/7/06 add datepicker.js
 * 
...
public class WebPageWriter {
	...
	static String productName = "Grand Unified Spreadsheet";
	...
	public WebPageWriter(SessionMgr s, WebLineWriter pw,
			HttpServletRequest parmReq, HttpServletResponse parmResp) {
		...
		httpOffset = (String) sm.getServletContext().getInitParameter(
				"HTTP-Offset");
	 ...
	 */
	public WebPageWriter(WebLineWriter pw, HttpServletRequest parmReq,
			HttpServletResponse parmResp) {
	...
	public void writePage(FormWriter fw) {
		...
		logging_in = logging_in
				|| formClassName.equalsIgnoreCase("forms.LoginForm");
		...
		try {
			...
			if (!formClassName.equalsIgnoreCase("forms.LoginForm")) {
				out
						.println("<script src=\""
								+ httpOffset
								+ "javascript/validate.js\"  type=\"text/javascript\"></script>");

				out
						.println("<script src=\""
								+ httpOffset
								+ "javascript/ajax.js\"  type=\"text/javascript\"></script>");
			...
			// used for dynamic menus.. not used now
			// out.println("<script src=\"" + httpOffset
			// + "Menu.js\" type=\"text/javascript\"></script>");

			out.print("<link rel=\"stylesheet\" href=\"" + httpOffset
					+ "styles/");
			...
			/*
			 * dump JavaScript from the Plugin.. stuff like date and integer
			 * edits.
			 * 
			 */

			// TAG:GUS2.0 REMOVE JAVASCRIPT START AND END TAGS <!-- ... --> FOR  HTML5
			out.println("<script >\n");
			...
			out.println("</HEAD>");
			out
					.println("<BODY topmargin=0 marginheight=0 leftmargin=0 marginwidth=0 onLoad=tmInit()> ");
			...
			out
					.println("<TABLE  width='100%' height='100%' BORDER='0' CELLSPACING='0' CELLPADDING='0' >");
			...
			// class="midalign"></img>
			out
					.println("<TR  HEIGHT=26>"
							+ "<td><TABLE WIDTH=100% BORDER='0' CELLSPACING='0' CELLPADDING='0'><TR>"
							+ "<TD bgcolor=white ALIGN=LEFT VALIGN=MIDDLE WIDTH=75><img src='"
							+ httpOffset
							...
							+ "<TD VALIGN=CENTER ALIGN=center><font size=4 color=#484848>"
							+ productName
							+ "</font></td>"
			...
			if (!logging_in) {
				out
						.println("<A class=blackButton HREF=Router?Target=Login&Action=Logout>Logout</a>");
			}

			out.println("&nbsp;&nbsp;</font></TD>" + "</TR></TABLE></td>"
					+ "</TR>");
			...
			out
					.println("<TR  bgcolor=#ededed HEIGHT=27><TD align=center width='100%' ALIGN=left>&nbsp;</td></tr>");
			...
			out
					.println("<TR bgcolor=#F0F0F0 HEIGHT=25>"
							+ "<TD><table width=100% BORDER='0' CELLSPACING='0' CELLPADDING='0'><tr>"
							+ "<td ALIGN=LEFT VALIGN=CENTER>"
							+ "<span class=regtext>&nbsp;&nbsp;"
							+ fw.getSubTitle() + "</span></td>"
							+ "<TD align=right>");

			// print Home link if not already there.
			if (!logging_in) {
				if (!sm.getTarget().equalsIgnoreCase("home")
						&& !sm.getTarget().equalsIgnoreCase("register")
						&& !sm.Parm("Action").equalsIgnoreCase("sendpassword")) {

					out
							.println("<A class=blackButton HREF=Router?Target=guess&Action=list>Home</a>&nbsp;&nbsp;");
			...
			if (!fw.isAdminFunction() && !logging_in) {
				...
				out
						.println("<A class=blackButton HREF=Router?Target=Project&Action=List>Dashboard</a>&nbsp;&nbsp;");

				if (sm.getTarget().equalsIgnoreCase("preference")) {
					out
							.println("<A class=blackButton HREF=Router?Target=Password&Action=Edit&Relation=this&RowKey="
									+ sm.getUserId().toString()
									+ ">Password</a>&nbsp;&nbsp;");
				} else {
					out
							.println("<A class=blackButton HREF=Router?Target=Preference&Action=Show&Relation=this&RowKey="
									+ sm.getUserId().toString()
									+ ">Preferences</a>&nbsp;&nbsp;");
				}
			}
			...
			if (!logging_in && !fw.isAdminFunction()
					&& sm.userIsAdministrator()) {
				out
						.println("<A class=blackButton HREF=Router?Target=Options&Action=Show&Relation=this&RowKey=1>System Admin.</a>"
								+ "&nbsp;&nbsp;");
			...
			out.println("<tr valign=top bgcolor=#ffffff><td><img src='"
					+ httpOffset
			...
			out
					.println("<TR  bgcolor=#ffffff HEIGHT=10><TD width='100%' ALIGN=left>&nbsp;</td></tr>");
			...
			out
					.println("<table border=0 cellpadding=0 cellspacing=0><tr bgcolor=#FFFFFF>");
			...
			out.println("</TD></TR>");
			out
					.println("<TR  class='copyright' align=bottom HEIGHT='30px'><TD align=center bgcolor=#ededed >"
							+ copyright
							+ (logging_in ? sponsored : "")
							+ "</TD></TR>");
			out.print("</TABLE>");
			...
			System.out
					.println("WebPageWriter:writePage fw : " + ioe.toString());
		}
	...
	public void printPage(FormWriter fw) {
		...
		try {
			...
			/*
			 * dump out the JavaScript from the Plugin.. stuff like date and
			 * integer edits.
			 */
			...
			out
					.println("<TABLE  class='tmtable' width='100%' height='100%'  >");
			...
			System.out
					.println("WebPageWriter:printPage fw : " + ioe.toString());
		}
	...
	private void writeToolbar(FormWriter fw) {

		out.println("<SCRIPT >");
		out.println("var http_offset='" + httpOffset + "';");
		out.println("var menu_name='" + fw.getMenuName() + "';");
		out.println("</SCRIPT>");

		try {
			// only show menu if logged in
			if (!formClassName.equalsIgnoreCase("forms.LoginForm_aaa")) {
				writeResource("/text/menu.js", out);
			}

		} catch (Exception ioe) {
			System.out.println("WriteMenuBar - caught exception:  "
					+ ioe.toString());
		}
	 ...
	 * 
	 * RD will look at the file and try to instantiate objects if it sees stuff
	 * like src="", which is in the menuLoader.js files.
	 * 
	 * So use this read/write instead to bypass RequestDispatch processing
	 * 
	 * the resource should be a path in a jar file in the lib directory,
	 * parallel to the web-inf classes.
	 */
	public void writeResource(String resource, WebLineWriter pw) {
		...
		try {
			...
			pw.println("WebPageWritter : Error writting source file :  "
					+ ioe.toString());
		}
	...
	public void debug(String debugMsg) {
		...
		if (this.debug)
			sm.debug(debugMsg);
		Logger.getLogger("gus").debug(debugMsg);
	}
